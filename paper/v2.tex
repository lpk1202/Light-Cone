...
\begin{document}
\title{Large Scale Structure Reconstruction with Short-Wavelength Modes: \\Light Cone Formalism and Redshift Space Distortions}
...
\bea
d_{{\rm m},V}(\vk) &=&\int d^3\vrr  \, d_{{\rm m},V}(\vrr)  e^{-i\vk \cdot\vrr} \vs
&=&\int d^3\vrr  \, W(\vec{r})\dm(\vrr;a(r)) \frac{D_{\ini}}{D_{1}(a(r))}  e^{-i\vk \cdot\vrr}
\eql{LCM}
\eea
We can make progress by writing
\be 
\dm(\vrr;a(r)) =\int \frac{d^3 \vk}{(2\pi)^3}e^{i\vk\cdot\vrr}\dm(\vk;a(r))\eql{feq}
\ee 
so that
\bea
d_{{\rm m},V}(\vk) &=&\int \frac{d^3 \vk'}{(2\pi)^3}\, \int d^3\vrr \,  e^{i(\vk'-\vk) \cdot\vrr} \vs
&&\times  \left[W(\vec{r}) \frac{D_{\ini}}{D_{1}(a(r))}  
\, \dm(\vk';a(r))\right].
\eea
In the case of the snapshot the term in brackets had no $\vrr$ dependence, so the integral over space led to a Dirac delta function, and then the integral over $\vk'$ to a tautology. Now, though, we have an expression for an observable $d_{{\rm m},V}(\vk)$ in terms of a theoretical quantity $\dm(\vk';a(r))$ that cannot be observed but whose behavior is governed by perturbation theory as outlined in \ec{tayl}. 
Therefore, perturbation theory predicts that 
\be 
d_{{\rm m},V}(\vk) = d_{{\rm m},V}^{(1)}(\vk)+d_{{\rm m},V}^{(2)}(\vk)+\cdots
\ee 
with each order given by:
\bea
d_{{\rm m},V}^{(i)}(\vk) &=&\int \frac{d^3 \vk'}{(2\pi)^3}\, \int d^3\vrr \,  e^{i(\vk'-\vk) \cdot\vrr} \vs
&&\times  \left[W(\vec{r}) \frac{D_{\ini}}{D_{1}(a(r))}  
...
\bea
&&d_{{\rm m},V}^{(1)}(\vk) \vs
&=&\int \frac{d^3 \vk'}{(2\pi)^3}\, \int d^3\vrr \,  e^{i(\vk'-\vk) \cdot\vrr} \vs
&&\times  \left[W(\vec{r}) \dm^{(1)}(\vk';a_{\ini})\right]  \vs
&=& \delta_{{\rm m},V}^{(1)}(\vk;a_{\ini})\equiv\delta_{{\rm m},{\rm ini},V}^{(1)}(\vk)\eql{dfo}
\eea 
since there's no $\vrr$ dependence in the bracket given that $\dm^{(1)}(\vk';a(r))$ evolves as $(D_{1}(a(r))/D_{\ini})\delta_{\rm m,ini}^{(1)}(\vk')$. Thus with the definition of $\delta_{{\rm m},V}^{(1)}(\vk;a_{\ini})$ similar to \ec{LC}, finally we have the relation in \ec{dfo}. Using this relation further we have:
...
\be 
\langle d_{{\rm m},V}^{(1)}(\vk) d_{{\rm m},V}^{(1)}(\vk') \rangle = (2\pi)^3 \delta_{\rm D}(\vk+\vk')P_{{\rm m,ini},V}(k).\eql{lps}
\ee 
where $P_{\rm m,ini}$ is the linear matter power spectrum at the initial time $a_{\rm ini}$ with volume $V$:
\be 
P_{{\rm m,ini},V}(k) = ....
\ee 
This relation \ec{lps} tells us that the first order term of the modified light cone matter density contrast $d_{{\rm m},V}(\vk)$ (which corresponds to long-wavelength modes) still characterizes the linear evolution information of the light cone, since the redshift difference in the light cone has been cancelled by the extra factor $D_{\ini}/D_{1}(a(r))$. We define the light cone matter power spectrum $\mathcal{P}_{{\rm m,nl},V}^{}(k)$ as:
\be 
\langle d_{{\rm m},V} (\vk)d_{{\rm m},V}(\vk')\rangle = (2\pi)^3 \delta_{\rm D}(\vk+\vk')\mathcal{P}_{{\rm m,nl},V}(k) +\cdots\eql{2ptlc}
\ee 
...
There are also off-diagonal terms in \ec{2ptlc} which contains extremely valuable information, and we will look into them in the next section \ref{sec5}.
