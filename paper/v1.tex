...
Name & Symbol &Defining Relation\\
 ...
 \hline
 Linear Matter Power Spectrum &$P_{\rm m,lin}(k;a)$ & $\langle \delta^{(\rm 1)}_{\rm m}(\vk;a) \delta^{(\rm 1)}_{\rm m}(\vk';a) \rangle \equiv (2\pi)^3 \delta_{\rm D}(\vk+\vk')P_{\rm m,lin}(k;a)$\\
 Nonlinear Matter Power Spectrum &$P_{\rm m,nl}(k;a)$ & $\langle \delta^{}_{\rm m}(\vk;a) \delta^{}_{\rm m}(\vk';a) \rangle \equiv (2\pi)^3 \delta_{\rm D}(\vk+\vk')P_{\rm m,nl}(k;a)$\\
 Initial Matter Power Spectrum & $ P_{\rm m,ini}(k)$ & $P_{\rm m,ini}(k)=(D_{\ini}/D_{1}(a))^2P_{\rm m,lin}(k;a)$ or \ec{lps} \\
 Light Cone Matter Power Spectrum & $\mathcal{P}_{\rm m}^{}(k)$ & \ec{2ptlc}; $\langle d_{\rm m} (\vk)d_{\rm m}(\vk')\rangle \equiv(2\pi)^3 \delta_{\rm D}(\vk+\vk')\mathcal{P}_{\rm m}(k)$\\
 Approximate Light Cone Matter Power Spectrum & $\mathcal{P}_{\rm m}^{\rm h}(k)$ &  $\langle d_{\rm m}^{\rm h} (\vk)d_{\rm m}^{\rm h}(\vk')\rangle \equiv (2\pi)^3 \delta_{\rm D}(\vk+\vk')\mathcal{P}_{\rm m}^{\rm h}(k)$\\
 \hline
\end{tabular}
...
\eea
\peikai{The first order term satisfies:
\be 
\dlc^{(1)}(\vk)=\delta_{\rm m}^{(1)}(\vk;a_{\ini})\equiv\delta_{\rm m,ini}^{(1)}(\vk)\eql{dfo}
\ee 
since there's no $\vrr$ dependence in the integrand given that $\dm^{(1)}(\vk';a(r))$ evolves as $(D_{1}(a(r))/D_{\ini})\delta_{\rm m,ini}^{(1)}(\vk')$. Thus the integral over $\vrr$ will result in a Dirac delta function, and then we have the relation in \ec{dfo}. Using this relation further we have:
...
\be 
\langle d^{(\rm 1)}_{\rm m}(\vk) d^{(\rm 1)}_{\rm m}(\vk') \rangle = (2\pi)^3 \delta_{\rm D}(\vk+\vk')P_{\rm m,ini}(k).\eql{lps}
\ee 
where $P_{\rm m,ini}$ is the linear matter power spectrum at the initial time $a_{\rm ini}$.} This relation \ec{lps} tells us that the first order term of the modified light cone matter density contrast $d_{\rm m}(\vk)$ (which corresponds to long-wavelength modes) still characterizes the linear evolution information of the light cone, since the redshift difference in the light cone has been cancelled by the extra factor $D_{\ini}/D_{1}(a(r))$. We define the light cone matter power spectrum $\mathcal{P}_{\rm m}^{}(k)$ as:
\be 
\langle d_{\rm m} (\vk)d_{\rm m}(\vk')\rangle = (2\pi)^3 \delta_{\rm D}(\vk+\vk')\mathcal{P}_{\rm m}(k) +\cdots\eql{2ptlc}
\ee 
...
Using the expressions of \ec{exp}, we can compute the two-point correlations of two short-wavelength Fourier modes $d_{\rm m}(\vk_s)$ and $d_{\rm m}(\vk_s')$ \peikai{up to second order}. Again we assume the squeezed limit $\vk_l=\vk_s+\vk_s'$:
\bea 
 &&\langle d_{\rm m}(\vk_s)d_{\rm m}(\vk_s') \rangle|_{\vk_s+\vk_s'=\vk_l}\vs
 =&&\langle d^{(1)}_{\rm m}(\vk_s)d^{(2)}_{\rm m}(\vk_s') \rangle+\langle d^{(2)}_{\rm m}(\vk_s)d^{(1)}_{\rm m}(\vk_s') \rangle \vs
 =&&\langle \dmi^{(1)}(\vk_s)d^{(2)}_{\rm m}(\vk_s') \rangle+\langle d^{(2)}_{\rm m}(\vk_s)\dmi^{(1)}(\vk_s') \rangle . \eql{2p}
\eea 
Here we make use of \ec{dfo} in the second step. Substituting \ec{exp} into \ec{2p} and evaluating the first bracket as an example:
\bea 
&&\langle \dmi^{(1)}(\vk_s)d^{(2)}_{\rm m}(\vk_s') \rangle \vs 
&=& \int \frac{d^{3}\vk'}{(2\pi)^3} \int_{V} d^{3} \vrr\,e^{i(\vk'-\vk_s')\cdot \vrr} \frac{D_{\ini}}{D_{1}(a(r'))}  \vs
...
&=&\langle \dmi^{(1)}(\vk_s)d^{(2)}_{\rm m}(\vk_s') \rangle \vs 
&\simeq &2C\int {d^{3}\vk'}\delta_{\rm D}(\vk'-\vk_s') \vs
&&\times F_{2}(-\vk_s,\vk_s+\vk')P_{\rm m,\ini}(k_s)\dmi^{(1)}(\vk_s+\vk') \vs
...
\eea 
Note that $\mathpzc{f}$ here differs from the $f$ defined in \ec{deff} by a factor of $C$ \peikai{$C\,(D_{\ini}/D_{1}(a))^2$} only.
