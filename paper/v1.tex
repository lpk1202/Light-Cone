...
\eea 
where we use the definition of the the linear growth factor $D_1(a(r))$. Plugging \ec{odb} into \ec{od}, we can see that the only $\vrr'$ dependent integral can be written as (here we simply choose a cube volume):
\be 
...
&= &2C\,F_{2}(-\vk_s,\vk_s+\vk_s')P_{\rm m,\ini}(k_s)d_{\rm m}^{\rm LC,(1)}(\vk_s+\vk_s')
\eea 
where in the first step, we perform a redefinition of integration dummy variable. And we use an approximation in the second step in order to get the first order term $d_{\rm m}^{\rm LC,(1)}(\vk_l)$ from the integral. 
...
\be 
\langle d^{\LC}_{\rm m}(\vk_s)d^{\LC}_{\rm m}(\vk_s') \rangle|_{\vk_s+\vk_s'=\vk_l} =\mathpzc{f}(\vec{k}_s,\vec{k}_s'){d}^{\rm LC,(1)}_{\rm m}(\vec{k}_l) \eql{2pt}
\ee 
with 
\bea
...
\be
\frac{1}{\sigma^{\rm LC}(k_l)^2}=\frac{V k_l^2 \Delta k }{(2\pi)^2}\bigg[\frac{P_{\rm m,lin}(k_l)}{P_{\rm m,lin}(k_l)+\mathpzc{A}(k_l) }\bigg]^2 \eql{error1},
\ee
Using the quadratic estimator \ec{estlc} we can use small scale information of the whole light cone (for now we only consider a cube-shaped volume) to infer large scale field of \ec{LC}. According to \ec{lps}, this is equivalent to linear matter perturbations in the light cone.
