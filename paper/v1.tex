...
\Sfig{hm}{$P_{\rm m}^{\rm h}(k)$ is the approximate matter power spectrum using halo information \ec{hm} at a snapshot when the scale factor $a=1$. We use the halo catalog from BIGMDPL, see section \ref{sec3} for a more detailed description.}

\Sfig{real_comp}{Comparison between the real matter density contrast $\delta_{\rm m}(\vrr)$ and the approximate matter density contrast using halo information $\dmh(\vrr)$ at a snapshot when the scale factor $a=1$. We use the halo catalog from BIGMDPL, see section \ref{sec3} for a more detailed description.}
