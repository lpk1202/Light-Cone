...
\be 
\langle d^{(\rm 1)}_{\rm m}(\vk) d^{(\rm 1)}_{\rm m}(\vk') \rangle \equiv (2\pi)^3 \delta_{\rm D}(\vk+\vk')P_{\rm m,ini}(k).\eql{lps}
\ee 
This relation \ec{lps} tells us that the first order term of the modified light cone matter density contrast $d_{\rm m}(\vk)$ (which corresponds to long-wavelength modes) still characterizes the linear evolution information of the light cone, since the redshift difference in the light cone has been cancelled by the extra factor $D_{\ini}/D_{1}(a(r))$. We define the light cone matter power spectrum $\mathcal{P}_{\rm m}^{}(k)$ as:
\be 
\langle d_{\rm m} (\vk)d_{\rm m}(\vk')\rangle \equiv (2\pi)^3 \delta_{\rm D}(\vk+\vk')\mathcal{P}_{\rm m}(k) \eql{2ptlc}
\ee 
...
\ee 
since $D_1(a(r'))$ is a slowly varying function. See appendix \ref{appenda} for a more detailed analysis. Constant $C$ can be further determined via integrating over $\vk\, '$ on both sides of \ec{dt}, which gives:
\be 
